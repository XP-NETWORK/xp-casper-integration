...
use crate::errors::UserNftStoreError;
...
pub fn get_named_arg_with_user_errors<T: FromBytes>(
    name: &str,
    missing: UserNftStoreError,
    invalid: UserNftStoreError,
) -> Result<T, UserNftStoreError> {
    ...
    let arg_bytes = if arg_size > 0 {
        ...
        res.unwrap_or_revert_with(UserNftStoreError::FailedToGetArgBytes)
    } else {
...
pub fn get_uref(name: &str, missing: UserNftStoreError, invalid: UserNftStoreError) -> URef {
    let key = get_key_with_user_errors(name, missing, invalid);
    key.into_uref()
        .unwrap_or_revert_with(UserNftStoreError::UnexpectedKeyVariant)
}
