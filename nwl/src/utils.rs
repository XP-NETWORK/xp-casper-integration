...
pub fn get_named_arg_with_user_errors<T: FromBytes>(
    ...
    let arg_bytes = if arg_size > 0 {
        ...
        res.unwrap_or_revert_with(NwlError::FailedToGetArgBytes)
    } else {
...
pub(crate) fn get_key_with_user_errors(name: &str, missing: NwlError, invalid: NwlError) -> Key {
    let (name_ptr, name_size, _bytes) = to_ptr(name);
    let mut key_bytes = vec![0u8; Key::max_serialized_length()];
    let mut total_bytes: usize = 0;
    let ret = unsafe {
...
pub fn get_uref(name: &str, missing: NwlError, invalid: NwlError) -> URef {
    let key = get_key_with_user_errors(name, missing, invalid);
    key.into_uref()
        .unwrap_or_revert_with(NwlError::UnexpectedKeyVariant)
}
